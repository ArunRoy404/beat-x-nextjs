...
function getActivityColor(type) {
  if (!type) return "var(--secondary)";
  if (type.endsWith(".rejected")) return "var(--red-error)";
  if (type.endsWith(".approved") || type === "payout.paid") return "var(--green-success)";
  if (type.startsWith("support.") || type === "payout.created" || type === "verification.submitted") return "var(--yellow-warning)";
  if (type.startsWith("user.")) return "var(--secondary)";
  return "var(--primary)";
}
...
const AdminDashboardOverviewPage = () => {
  ...
  const greetingData = useMemo(() => {
    const pendingReviewsCount = data?.pendingContentReview?.length ?? 0;

    return {
      greeting: "Welcome Back, Admin!",
      statusLabel: "System Active",
      activeUsersText: `${(data?.statCards?.activeUsers ?? 0).toLocaleString()} Users`,
      activeUsersLabel: "active now",
      pendingReportsText: `${(data?.statCards?.pendingVerification ?? 0).toLocaleString()} Pending`,
      pendingReportsLabel: "verifications",
      pendingReviewsText: pendingReviewsCount > 0 ? `${pendingReviewsCount.toLocaleString()} Pending` : null,
      pendingReviewsLabel: "reviews",
      operationalText: "All Services Operational",
      metrics: [
        { label: "Active Artists", value: `${(data?.statCards?.activeArtists ?? 0).toLocaleString()}`, type: "primary" },
        { label: "Pending Payouts", value: `${(data?.statCards?.pendingPayoutsCount ?? 0).toLocaleString()}`, type: "warning" },
        { label: "Revenue MTD", value: `৳${Number(data?.statCards?.revenueMTD ?? 0).toLocaleString()}`, type: "success" },
      ],
    };
  }, [data]);
    ...
    return [
      {
        id: "total-users",
        title: "Total Users",
        value: `${(data?.statCards?.totalUsers ?? 0).toLocaleString()}`,
        icon: "Users",
      ...
      },
      {
        id: "total-songs",
        title: "Total Songs",
        value: `${(data?.statCards?.totalSongs ?? 0).toLocaleString()}`,
        icon: "Music",
      ...
      },
      {
        ...
        value: `${(data?.statCards?.activeArtists ?? 0).toLocaleString()}`,
        icon: "UserCheck",
      ...
      },
      {
        ...
        value: `${((data?.statCards?.totalAudiobooks ?? 0) + (data?.statCards?.totalPodcasts ?? 0)).toLocaleString()}`,
        icon: "Headphones",
  ...
  const recentUploadsData = useMemo(() => {
    return (data?.recentUploads || []).map((upload, idx) => {
      ...
      const status =
        rawStatus === "active" || rawStatus === "published"
          ? "Published"
          : rawStatus === "scheduled"
          ? "Scheduled"
          : "Draft";
      ...
      return {
        ...
        plays: `${(upload?.playCount ?? 0).toLocaleString()} plays`,
        status,
      };
    });
  }, [data]);
  ...
  const upcomingEventsData = useMemo(() => {
    return (data?.upcomingEvents || []).map((event, idx) => {
      ...
      return {
        ...
        location,
        ticketsSold: `${(event?.ticketsSold ?? 0).toLocaleString()}/${(event?.totalTickets ?? 0).toLocaleString()} tickets sold`,
        status: (event?.status || "").toLowerCase() === "active" ? "Upcoming" : event?.status || "Scheduled",
      };
    });
  }, [data]);
