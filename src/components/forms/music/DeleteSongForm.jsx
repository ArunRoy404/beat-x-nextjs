...
import { useDeleteSong } from "@/hooks/api/admin/songs/useDeleteSong"
    ...
    const handleDelete = () => {
        deleteSong(
            { id: song._id },
            {
                ...
                onError: (error) => toast.error(error?.message || "Failed to delete song."),
            }
        )
    }
    ...
    return (
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4 text-left">
            <p className="text-light-whitetext text-[14px] not-italic font-normal leading-[22px] font-sans">
                You are about to permanently delete <span className="text-whitetext font-semibold">{song?.title}</span>. This action cannot be undone.
            </p>
