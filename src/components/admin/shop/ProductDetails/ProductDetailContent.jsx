  ...
  return (
    <div className="p-4 flex flex-col gap-5 overflow-y-auto flex-1 min-h-0 scrollbar-thin">
      ...
      <div className="grid grid-cols-2 gap-4 shrink-0">
        ...
        <CommonInfoBox label="Price" value={`৳${product?.price ?? 0}`} />
        <CommonInfoBox label="In Stock" value={product?.stock ?? 0} />
        <CommonInfoBox label="Units Sold" value={product?.soldCount ?? product?.sold ?? 0} />
        <CommonInfoBox label="Coin Reward" value={`${product?.coinReward ?? 0} coins`} />
        <CommonInfoBox label="Artist" value={product?.artistName || product?.ownerId?.name || "Various"} />
      </div>
