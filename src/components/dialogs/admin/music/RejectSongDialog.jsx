...
import { useRejectSong } from "@/hooks/api/admin/songs/useRejectSong";
import { toast } from "sonner";

const RejectSongDialog = ({ song, children }) => {
  const [open, setOpen] = useState(false);
  const [reason, setReason] = useState("Audio quality too low.");
  const { mutate: rejectSong, isPending } = useRejectSong();
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3">
            ...
            <span className="text-[18px] font-semibold leading-none">Reject Song Submission</span>
          </DialogTitle>
        </DialogHeader>

        <form onSubmit={handleReject} className="flex flex-col gap-4 mt-2">
          <p className="text-sm text-light-gray">
            Are you sure you want to reject <span className="text-whitetext font-semibold">{song?.title || "this song"}</span>? Please provide a reason for the artist.
          </p>
...
export default RejectSongDialog;
