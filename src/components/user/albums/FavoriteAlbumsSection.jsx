...
const FavoriteAlbumsSection = ({ featured, albums = [] }) => {
    if (!featured && (!albums || albums.length === 0)) {
        return (
            <section className="flex w-full flex-col gap-4">
                <h2 className="text-2xl text-whitetext sm:text-[32px]">Albums</h2>
                <div className="flex w-full flex-col items-center justify-center rounded-[16px] border border-dashed border-white/10 py-12 text-center">
                    <p className="text-base text-light-gray">No albums found</p>
                    <p className="mt-1 text-sm text-light-gray/60">Discover new releases and save your favorite albums to view them here.</p>
                </div>
            </section>
        )
    }
                    ...
                    <div className="relative flex min-h-100 w-full flex-col justify-end gap-4 overflow-hidden rounded-[16px] border border-light-gray p-6 lg:min-h-0 lg:flex-[1_0_0]">
                        ...
                        <div className="relative flex flex-col gap-2">
                            ...
                            <p className="truncate text-base font-semibold">
                                ...
                                {featured?.meta && <span className="text-light-gray">• {featured.meta}</span>}
                            </p>
                        </div>
                        ...
                        <div className="relative flex items-center gap-4">
                            <button
                                type="button"
                                className="flex cursor-pointer items-center gap-2 rounded-full bg-secondary px-8 py-4 text-base font-semibold text-button-text transition-transform active:scale-95"
                            >
                ...
                {albums?.length > 0 && (
                    <div className="grid w-full grid-cols-2 gap-4 lg:w-135 lg:shrink-0">
                        {albums.map((album, idx) => (
                            <RankedChartCard key={album?.id || album?._id || idx} item={album} showRank={false} />
                        ))}
                    </div>
                )}
            </div>
