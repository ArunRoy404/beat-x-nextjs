...
const rejectionReasons = [
    { value: "venue_capacity_unclear", label: "Venue capacity documentation unclear" },
    { value: "duplicate_event", label: "Duplicate or conflicting event listing" },
    { value: "pricing_issue", label: "Ticket pricing / policy issue" },
...
const EventUnderReviewDialogFooter = ({ event, startWithRejection = false }) => {
    const [isRejecting, setIsRejecting] = useState(startWithRejection)
    const [adminNote, setAdminNote] = useState("")
    const [rejectionReason, setRejectionReason] = useState("venue_capacity_unclear")
    const [additionalNote, setAdditionalNote] = useState("")

    // Sync isRejecting when startWithRejection changes, without an effect
    // (React's recommended "adjust state during render" pattern).
    const [prevStartWithRejection, setPrevStartWithRejection] = useState(startWithRejection)
    if (startWithRejection !== prevStartWithRejection) {
        setPrevStartWithRejection(startWithRejection)
        setIsRejecting(startWithRejection)
    }
...
export default EventUnderReviewDialogFooter
