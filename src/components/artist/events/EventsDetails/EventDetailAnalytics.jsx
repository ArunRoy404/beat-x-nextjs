...
import { useArtistEventDetailsAnalyticsStore } from "@/zustandStore/artist/artistStore/artistEventDetailsAnalyticsStore"
import DashboardStats from "@/components/shared/Dashboard/DashboardStats/DashboardStats"
import { ResponsiveContainer, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip } from "recharts"

const EventDetailAnalytics = () => {
    const eventStatsCards = useArtistEventDetailsAnalyticsStore((state) => state.eventStatsCards)
    const eventPerformanceData = useArtistEventDetailsAnalyticsStore((state) => state.eventPerformanceData)
    const eventPlatformData = useArtistEventDetailsAnalyticsStore((state) => state.eventPlatformData)
    const eventCountryData = useArtistEventDetailsAnalyticsStore((state) => state.eventCountryData)
                ...
                <div style={{ height: "200px", minHeight: "200px" }} className="w-full z-10 relative">
                    <ResponsiveContainer width="100%" height="100%" minHeight={200} debounce={1000}>
                        <AreaChart data={eventPerformanceData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                            <defs>
                                <linearGradient id="colorArtistEventSales" x1="0" y1="0" x2="0" y2="1">
                            ...
                            <Area
                                ...
                                strokeWidth={2}
                                fillOpacity={1}
                                fill="url(#colorArtistEventSales)"
