...
import { useArtistPodcastDetailsAnalyticsStore } from "@/zustandStore/artist/artistStore/artistPodcastDetailsAnalyticsStore"
import DashboardStats from "@/components/shared/Dashboard/DashboardStats/DashboardStats"
import { ResponsiveContainer, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip } from "recharts"

const PodcastDetailAnalytics = () => {
    const podcastStatsCards = useArtistPodcastDetailsAnalyticsStore((state) => state.podcastStatsCards)
    const podcastPerformanceData = useArtistPodcastDetailsAnalyticsStore((state) => state.podcastPerformanceData)
    const podcastPlatformData = useArtistPodcastDetailsAnalyticsStore((state) => state.podcastPlatformData)
    const podcastCountryData = useArtistPodcastDetailsAnalyticsStore((state) => state.podcastCountryData)
                ...
                <div style={{ height: "200px", minHeight: "200px" }} className="w-full z-10 relative">
                    <ResponsiveContainer width="100%" height="100%" minHeight={200} debounce={1000}>
                        <AreaChart data={podcastPerformanceData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                            <defs>
                                <linearGradient id="colorArtistPodcastStream" x1="0" y1="0" x2="0" y2="1">
                            ...
                            <Area
                                ...
                                strokeWidth={2}
                                fillOpacity={1}
                                fill="url(#colorArtistPodcastStream)"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="relative overflow-hidden rounded-[16px] border border-white/10 p-5 bg-[#0E0E0E]">
                    ...
                    <div className="flex flex-col gap-3 relative z-10">
                        {podcastCountryData.map((country) => (
